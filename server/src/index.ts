...
import { loadServerBuildInfo } from "./build-info";

const config = loadConfig();
const buildInfo = loadServerBuildInfo();
...
try {
  ...
  app.log.info({
    ...
    buildCommit: buildInfo.commit?.slice(0, 12) ?? null,
    protocolVersion: PROTOCOL_VERSION,
  }, `OpenCord Server listening on http://${config.HOST}:${config.PORT}`);
} catch (error) {
  app.log.error(error);
  process.exitCode = 1;
  await app.close();
