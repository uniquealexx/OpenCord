...
Mute and deafen can be bound to system-wide shortcuts that work even when OpenCord has no focus (desktop shell only — the Android bridge has no `keybinds` surface). The main process runs a passive global keyboard hook (`uiohook-napi`, prebuilt N-API binaries, `asarUnpack`'ed in packaged builds). The hook is a listener only: it never swallows keystrokes, so bound combinations still reach other applications. `KeybindController` (`client/electron/keybinds.ts`) maps raw scancodes to `KeyboardEvent.code` names, deduplicates OS auto-repeat, tracks modifiers, and emits `{action, mode, phase}` events over the `keybinds:action` IPC channel; every incoming payload is validated with Zod schemas from `client/src/shared/keybinds.ts`. The renderer (`useKeybindActions`) applies events to the voice session: presses work in an active session, releases always apply so a held key never leaves a stuck mute, and unmuting is blocked while a server-side mute is in force — the same guard as the UI buttons. The bind map (per action: physical key + modifiers + `toggle`/`hold` mode) is stored client-side in `preferences.keybinds` of `client-state.json` and pushed to main on every change; while the settings dialog captures a key it suppresses dispatch, so assigning a combination does not trigger it. A lone modifier (Alt, Ctrl, Shift, Win/Cmd — left and right sides count as one) is a valid bind on its own: press and release it without other keys during capture. To keep everyday shortcuts safe, a lone-modifier `toggle` fires on key release and only when no other key was pressed in between, so Alt+Tab never flips the mute; `hold` presses on keydown and releases on keyup as usual. Keys are used solely for matching against binds — they are not logged or sent anywhere. macOS requires an Accessibility permission; Wayland (Linux) is not supported by the hook.
