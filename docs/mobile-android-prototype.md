...
3. **Мобильная адаптация UI**: строка заголовка окна скрывается (системный статус-бар), список каналов и список участников становятся накладками поверх чата (кнопки в шапке канала), safe-area отступы через `env(safe-area-inset-*)`. Экранные формы адаптированы под телефоны 320–430px и планшеты без изменения десктопа: диалоги занимают полную ширину с внутренней прокруткой (`max-sm:`, `dvh`), onboarding без «карточки в окне» (переключатель языка — в потоке формы и всегда видим), поиск по серверу — полноэкранная панель, эмодзи-панель фиксируется над композером, длинные статусы/имена обрезаются (важные атрибуты сохраняются), поля ввода на узких экранах получают 16px (анти-зум WebView). В настройках на мобильных доступен масштаб интерфейса 90–120% (применяется через `zoom` к корню документа, поэтому масштабируются и порталы-диалоги; на десктопе не отображается и не применяется).
4. **Разрешения**: `RECORD_AUDIO`/`MODIFY_AUDIO_SETTINGS` объявлены в манифесте; при входе в приложение клиент запрашивает доступ к микрофону системным диалогом Android (поток сразу останавливается) и повторяет запрос при следующих запусках, пока разрешение не предоставлено.
