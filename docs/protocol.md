...
Protocol v40 adds the optional `participantLimit` field to `channel.create` — values `0–25` for voice channels (`0` means unlimited), `null` for text channels — so the capacity can be chosen at creation instead of only afterwards via `channel.update`. The field is optional, so an older client that omits it still creates a voice channel with the default limit of 25.

Protocol v39 adds the `nameFont` profile field — one of `none`, `pixel`, `gothic`, `italic`, `mono`, or `serif` — with which the client renders the nickname in a different typeface wherever it is shown. The username text itself is unchanged; the font is client-side CSS only. It travels through `auth.respond.profile`, `profile.update`, `server.snapshot.members`, and `member.updated`, defaulting to `none`, so a server of an earlier version stays compatible.
...
The username, description, avatar, banner, and member list background are returned in `server.snapshot.members` and `member.updated`; the current username and avatar are also used by events and the message history. Therefore one server profile is used by the member list, the text chat, and the voice room interface, while the banner is shown in the profile preview that opens and the member list background is shown behind the user's own row in the member list. When the profile or status changes, the client sends `profile.update` over the existing WebSocket without reconnecting. The server replaces the previous public fields in the user's single record and broadcasts `member.updated` to all active clients. On explicit leave, the public description, avatar, and banner are cleared on the server. Since protocol v37 the profile also carries the optional `accentColor` — a `#rrggbb` value without alpha with which the client tints the glass of the profile preview card; since v38 it additionally carries `nameGlow` in the same format, drawing a soft diffuse glow around the nickname; since v39 it carries `nameFont` (one of `none`, `pixel`, `gothic`, `italic`, `mono`, `serif`, defaulting to `none`), rendering the nickname in a different typeface without changing its text; since v41 it carries `memberBackground` — a WebP image in the banner format (5:2, at most 600×240, up to 256 KB) shown behind the user's own row in the member list. All four fields are decorative, are not included in `bannedMembers`, and are cleared together with the rest of the public fields.
...
Voice messages are audio attachments (`audio/webm`, `audio/ogg`, `audio/mpeg`, `audio/mp4`, `audio/wav`) recorded with `MediaRecorder` in the composer (300 s cap on desktop, 120 s on mobile) and sent through the regular attachment upload (`x-opencord-file-name: voice-message-YYYYMMDD-HHmmss.<ext>`). The recorder MIME may carry codec parameters (`audio/webm;codecs=opus`) — the clients strip them before upload so the stored type stays `audio/*`. A message renders as a voice bubble only when both the audio MIME and the `voice-message-<date>-<time>.<ext>` file name match; any other audio file (e.g. `song.mp3`) renders as a plain download row. No protocol change: duration is player-side metadata, search classifies voice as `file`, and the default size limit applies. The desktop client caches audio previews as verified local files like video; the Android prototype streams previews up to 10 MB as data URLs.
...
Протокол v40 добавляет в `channel.create` необязательное поле `participantLimit` — значения `0–25` для голосовых каналов (`0` — без ограничения), `null` для текстовых, — чтобы лимит можно было выбрать сразу при создании, а не только потом через `channel.update`. Поле необязательное, поэтому старый клиент без него по-прежнему создаёт голосовой канал с лимитом по умолчанию 25.

Протокол v39 добавляет поле профиля `nameFont` — одно из `none`, `pixel`, `gothic`, `italic`, `mono` или `serif`, — которым клиент отрисовывает ник другим шрифтом везде, где он отображается. Сам текст username не меняется; шрифт — только CSS на клиенте. Поле передаётся через `auth.respond.profile`, `profile.update`, `server.snapshot.members` и `member.updated` с дефолтом `none`, поэтому сервер прошлой версии остаётся совместимым.
...
`username`, описание, аватар, шапка и фон в списке участников возвращаются в `server.snapshot.members` и `member.updated`; актуальные имя и аватар также используются событиями и историей сообщений. Поэтому один серверный профиль используется списком участников, текстовым чатом и интерфейсом голосовой комнаты, а шапка показывается в открываемом превью профиля, а фон — за собственной строкой пользователя в списке участников. При смене профиля или статуса клиент отправляет `profile.update` по существующему WebSocket без переподключения. Сервер заменяет прежние публичные поля в единственной записи пользователя и рассылает `member.updated` всем активным клиентам. При явном выходе публичные описание, аватар и шапка очищаются на сервере. Начиная с протокола v37 профиль несёт также необязательный `accentColor` — значение `#rrggbb` без альфы, которым клиент окрашивает стекло карточки превью профиля; с v38 в том же формате передаётся и `nameGlow`, рисующее мягкое рассеянное свечение вокруг ника; с v39 — `nameFont` (одно из `none`, `pixel`, `gothic`, `italic`, `mono`, `serif` с дефолтом `none`), отрисовывающее ник другим шрифтом без изменения текста; с v41 — `memberBackground` (WebP-изображение в формате шапки 5:2, максимум 600×240, до 256 КБ), которое показывается за собственной строкой пользователя в списке участников. Все четыре поля декоративные, в `bannedMembers` не входят и очищаются вместе с остальными публичными полями.
...
При явном выходе клиент отправляет `server.leave`. Для обычного участника сервер удаляет членство, рассылает `member.removed` и ещё семь дней хранит публичный профиль, чтобы недавняя история оставалась читаемой. После дедлайна фоновая очистка меняет имя на «Неизвестный пользователь» и удаляет username, дискриминатор, описание, аватар, шапку, фон в списке участников, акцентный цвет и свечение ника, сбрасывая шрифт ника в `none`, сохраняя криптографическую идентичность и все сообщения. Повторный вход до или после очистки восстанавливает публичный профиль из авторизованного клиента. Владелец не удаляется без передачи владения или удаления сервера.
...
协议 v40 在 `channel.create` 中新增可选字段 `participantLimit`——语音频道取 `0–25`（`0` 表示无限制），文本频道取 `null`——以便在创建时即可选择容量，而不必事后再通过 `channel.update` 设置。该字段是可选的，因此省略它的旧客户端仍会以默认限制 25 创建语音频道。

协议 v39 新增了个人资料字段 `nameFont`——`none`、`pixel`、`gothic`、`italic`、`mono` 或 `serif` 之一——客户端用它在昵称的所有显示位置以不同字体渲染。用户名文本本身不变；字体仅是客户端 CSS。该字段通过 `auth.respond.profile`、`profile.update`、`server.snapshot.members` 和 `member.updated` 传输，默认值为 `none`，因此旧版本服务器保持兼容。
...
`username`、描述、头像、横幅和成员列表背景在 `server.snapshot.members` 和 `member.updated` 中返回；当前的 `username` 和头像也会被事件和消息历史使用。因此，一个服务器个人资料被成员列表、文本聊天和语音房间界面共同使用，而横幅显示在打开的个人资料预览中，成员列表背景显示在成员列表中用户自己一行的后面。当个人资料或状态发生变化时，客户端会通过现有 WebSocket 发送 `profile.update`，无需重新连接。服务器在用户的唯一记录中替换之前的公开字段，并向所有活动客户端广播 `member.updated`。在明确退出时，服务器会清除公开描述、头像和横幅。从协议 v37 开始，个人资料还携带可选的 `accentColor`——一个不带透明度的 `#rrggbb` 值，客户端用它为个人资料预览卡片的玻璃效果着色；从 v38 开始，相同格式还传递 `nameGlow`，为昵称绘制柔和的弥散光晕；从 v39 开始还传递 `nameFont`（`none`、`pixel`、`gothic`、`italic`、`mono`、`serif` 之一，默认为 `none`），以不同字体渲染昵称而不改变文本；从 v41 开始还传递 `memberBackground`（横幅格式的 WebP 图像，5:2，最大 600×240，不超过 256 KB），显示在成员列表中用户自己一行的后面。这四个字段都是装饰性的，不包含在 `bannedMembers` 中，并与其他公开字段一起清除。
...
明确退出时，客户端发送 `server.leave`。对于普通成员，服务器会移除成员资格、广播 `member.removed`，并保留公开资料七天，以便近期历史记录仍可阅读。保留期结束后，后台清理会将名称替换为“未知用户”，并清除用户名、识别码、简介、头像、横幅、成员列表背景、强调色和昵称光晕，将昵称字体重置为 `none`，同时保留加密身份及所有消息。用户在清理前后重新加入时，公开资料都会从已认证客户端恢复。所有者在未转让所有权或删除服务器的情况下不会被移除。
