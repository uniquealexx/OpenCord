...
Для подготовки release bundle из проверенного checkout используется команда `pnpm bundle:server`. Она создаёт игнорируемые Git файлы `release/opencord-server-<version>.tar.gz` и `.sha256`; публикация архива и контрольной суммы должна выполняться релизным процессом раздельно.
