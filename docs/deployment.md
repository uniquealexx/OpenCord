...
`update` не требует нового SSH-развёртывания из клиента. Команда загружает указанный HTTPS release bundle (либо читает `--bundle-file`), обязательно сверяет переданную SHA-256 сумму, проверяет структуру архива, создаёт backup PostgreSQL и вложений и запускает идемпотентный установщик Docker или native. Рабочие данные не удаляются. Автоматический канал `latest` намеренно не зафиксирован до появления официального репозитория релизов: администратор должен использовать URL и SHA-256 из одной конкретной доверенной публикации.

Формат будущего источника автоматических обновлений описан в [release-manifest.md](./release-manifest.md). На текущем этапе manifest создаётся и валидируется локально, но `opencordctl` и Electron ещё не скачивают его автоматически.

Владелец также может открыть меню сервера в клиенте и выбрать «Обновить сервер». Для экземпляров, изначально развёрнутых актуальным клиентом, адрес VPS, SSH-порт, пользователь, домен, email и выбранный режим уже заполнены. Клиент повторно загружает встроенный проверенный bundle и запускает идемпотентный установщик с сохранением PostgreSQL и вложений. Пароли, passphrase, fingerprint и приватные SSH-ключи не находятся в локальном состоянии, поэтому секрет требуется ввести или выбрать заново. Для старой записи сервера без сохранённой конфигурации кнопку нужно активировать одним повторным развёртыванием через мастер.
...
Electron и VPS-установщики проверяют внешний SHA-256. Установщики дополнительно проверяют метаданные и платформу bundle, размер и SHA-256 вложенного runtime, пути, типы записей и границы симлинков. Локальный `.sha256` обнаруживает повреждение, но не доказывает авторство релиза; подпись и доверенный GitHub manifest будут отдельным этапом.

В development клиент автоматически ищет `release/opencord-server-<version>.tar.gz`. Другой архив можно выбрать в окне развёртывания. В упакованном клиенте пока используется ручной выбор, а путь к файлу не сохраняется.
