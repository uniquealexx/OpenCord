...
Проверка запускается один раз через десять секунд после старта приложения и доступна
вручную в настройках. Автоматически загружается только manifest; NSIS installer
скачивается после нажатия пользователем кнопки, а установка и перезапуск требуют
...
сборки обеих частей, поэтому клиент не видит частично собранный новый релиз.
