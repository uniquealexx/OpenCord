...
type UpdateCheckResult = { updateInfo: { version: string } } | null;
...
export class ClientUpdateManager {
  ...
  constructor(
    ...
    this.state = enabled
      ...
      : { status: "disabled", currentVersion, channel, reason: "Обновления доступны только в установленной Windows-сборке OpenCord" };
    updater.autoDownload = false;
    updater.autoInstallOnAppQuit = false;
    updater.allowDowngrade = false;
    updater.on("download-progress", ({ percent }) => {
      if (!this.candidate || this.state.status !== "downloading") return;
      this.setState({ status: "downloading", currentVersion, channel, version: this.candidate.version, percent: Math.max(0, Math.min(100, percent)) });
    });
  ...
  check(): Promise<ClientUpdateState> {
    if (!this.enabled) return Promise.resolve(this.state);
