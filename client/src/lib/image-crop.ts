...
export function validateCropSource(source: Blob): void {
  ...
  if (source.size > MAX_CROP_SOURCE_BYTES) throw new Error("Исходное изображение должно быть меньше 20 МБ");
}
...
export function imageDataUrlToFile(dataUrl: string, fileName: string): File {
  ...
  if (!match) throw new Error("Не удалось прочитать установленное изображение");
