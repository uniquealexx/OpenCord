...
export function DeploymentDialog({ open, onOpenChange, onDeployed, preset, updateOnly = false }: DeploymentDialogProps): React.ReactElement {
  ...
  async function inspect(event: React.FormEvent): Promise<void> {
    ...
    try {
      const bridge = window.openCord?.deployment;
      if (!bridge) throw new Error("SSH API доступен только в desktop-приложении");
      if (authentication === "private-key" && !selectedKey) throw new Error("Выберите приватный SSH-ключ");
  ...
  function connectionPayload(): DeploymentConnection {
    if (!identity) throw new Error("Сначала проверьте SSH fingerprint");
  ...
  async function inspectEnvironment(): Promise<void> {
    ...
    try {
      const bridge = window.openCord?.deployment;
      if (!bridge) throw new Error("SSH API недоступен");
  ...
  return <Dialog open={open} onOpenChange={close}>
    <DialogContent className="max-w-2xl">
      ...
      {step === "configuration" && <form onSubmit={(event) => void inspect(event)} className="space-y-4">
        <Field label={ru.deployment.serverName}><Input value={serverName} onChange={(event) => setServerName(event.target.value)} minLength={2} maxLength={48} required placeholder={ru.deployment.serverNamePlaceholder} /></Field>
        <div className="grid grid-cols-[1fr_110px] gap-3">
          <Field label={ru.deployment.host}><Input value={host} onChange={(event) => setHost(event.target.value)} placeholder="203.0.113.10" required /></Field>
          <Field label={ru.deployment.port}><Input value={port} onChange={(event) => setPort(event.target.value)} type="number" min={1} max={65535} required /></Field>
        </div>
        <div className="grid grid-cols-2 gap-3">
          <Field label={ru.deployment.username}><Input value={username} onChange={(event) => setUsername(event.target.value)} placeholder="root" required /></Field>
          <Field label={ru.deployment.domain}><Input value={domain} onChange={(event) => { setDomain(event.target.value); if (!event.target.value.trim()) { setEmail(""); setInsecureConfirmed(false); } }} placeholder="chat.example.com" /></Field>
        </div>
        ...
        <Field label="OpenCord Server bundle">
          <span className="grid gap-1.5">
            <Button type="button" variant="secondary" onClick={() => void chooseBundle()} className="w-full">
              <FileArchive className="size-4" />{selectedBundle ? `${selectedBundle.fileName} · ${selectedBundle.version}` : "Использовать локальный файл вместо релиза"}
            </Button>
            <span className="text-[11px] font-normal text-slate-500">По умолчанию клиент скачает bundle своей версии из GitHub Releases и проверит manifest, размер, SHA-256 и внутренний runtime. Для ручного файла рядом должен находиться `.sha256`.</span>
          </span>
        </Field>
      ...
      {step === "fingerprint" && identity && <div className="space-y-5">
        <section className="rounded-2xl border border-amber-300/15 bg-amber-300/5 p-4">
          <div className="mb-2 flex items-center gap-2 font-semibold text-amber-100"><ShieldCheck className="size-5" />{ru.deployment.fingerprintTitle}</div>
          <p className="mb-4 text-xs leading-5 text-amber-100/60">{ru.deployment.fingerprintHint}</p>
        ...
        <label className="flex cursor-pointer items-start gap-3 rounded-xl border border-white/8 p-3 text-sm text-slate-300"><input type="checkbox" checked={fingerprintConfirmed} onChange={(event) => setFingerprintConfirmed(event.target.checked)} className="mt-0.5 size-4 accent-violet-500" />{ru.deployment.fingerprintConfirm}</label>
        {error && <ErrorMessage>{error}</ErrorMessage>}
        <div className="flex gap-3"><Button variant="secondary" onClick={() => setStep("configuration")} className="flex-1">Назад</Button><Button disabled={!fingerprintConfirmed || busy} onClick={() => void inspectEnvironment()} className="flex-1">{busy && <LoaderCircle className="size-4 animate-spin" />}{ru.deployment.inspectEnvironment}</Button></div>
      </div>}

      {step === "environment" && environment && <div className="space-y-4">
        <div><h3 className="text-base font-semibold text-slate-100">{ru.deployment.environmentTitle}</h3><p className="mt-1 text-xs leading-5 text-slate-500">{ru.deployment.environmentDescription}</p></div>
        <section className="grid grid-cols-2 gap-3 rounded-2xl border border-white/8 bg-black/15 p-4 text-xs">
          <div><div className="mb-1 text-slate-600">{ru.deployment.systemSummary}</div><div className="font-medium text-slate-200">{environment.osId} {environment.osVersion} · {environment.architecture}</div></div>
          <div><div className="mb-1 text-slate-600">systemd</div><div className={environment.systemd ? "text-emerald-300" : "text-red-300"}>{environment.systemd ? "доступен" : "не найден"}</div></div>
          <div><div className="mb-1 text-slate-600">Docker</div><div className={environment.dockerUsable ? "text-emerald-300" : "text-amber-300"}>{environment.dockerUsable ? ru.deployment.dockerReady : ru.deployment.dockerMissing}</div></div>
