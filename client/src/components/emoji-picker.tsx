  ...
  return <div ref={rootRef} className="relative shrink-0">
    ...
    {open && <div role="dialog" aria-label={t.emoji.panel} className="glass absolute bottom-12 right-0 z-50 w-[344px] overflow-hidden rounded-2xl shadow-[0_22px_70px_rgba(0,0,0,.55)] max-sm:fixed max-sm:inset-x-3 max-sm:bottom-20 max-sm:w-auto">
      ...
      <div className="flex h-12 items-center justify-between border-t border-white/[.07] px-2">
        <button type="button" aria-label={t.emoji.recentAria} title={t.emoji.recent} onClick={() => setCategory("recent")} className={cn("grid size-9 place-items-center rounded-lg text-xl grayscale hover:bg-white/[.06] hover:grayscale-0 max-sm:size-7 max-sm:text-base", category === "recent" && "bg-violet-400/15 grayscale-0")} style={{ fontFamily: emojiFont }}>🕘</button>
        {emojiCategories.map((item) => <button key={item.id} type="button" aria-label={t.emoji[item.id]} title={t.emoji[item.id]} onClick={() => setCategory(item.id)} className={cn("grid size-9 place-items-center rounded-lg text-xl grayscale hover:bg-white/[.06] hover:grayscale-0 max-sm:size-7 max-sm:text-base", category === item.id && "bg-violet-400/15 grayscale-0")} style={{ fontFamily: emojiFont }}>{item.icon}</button>)}
      </div>
    </div>}
  </div>;
}
