...
import { Check, ChevronDown, File, FileText, Film, Hash, ImageIcon, LoaderCircle, Paperclip, Play, Search, User, X } from "lucide-react";
...
export function ServerSearchPanel({ open, serverName, channels, members, result, loading, onClose, onSearch, onOpenMessage, previewAvailable, onPreview }: { open: boolean; serverName: string; channels: MockChannel[]; members: MockMember[]; result: MessageSearchResult | null; loading: boolean; onClose: () => void; onSearch: (filters: MessageSearchFilters) => void; onOpenMessage: (message: MockMessage) => void; previewAvailable: boolean; onPreview: (attachment: Attachment) => Promise<string> }): React.ReactElement | null {
...
/** Заменяет маркеры <@userId> на читаемые теги @username#1234; неизвестных — на «неизвестный пользователь». */
function readableContent(content: string, members: MockMember[], unknownLabel: string): string {
  const candidates: MentionCandidate[] = members.map((member) => ({ id: member.id, username: member.username, discriminator: member.discriminator, displayName: member.displayName, avatar: member.avatar ?? null, banner: member.banner ?? null, color: member.avatarColor, status: member.status, role: member.role, bio: member.bio, fingerprint: member.fingerprint }));
