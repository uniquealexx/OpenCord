...
export function SettingsDialog({ preferences, open, confirmReset, onOpenChange, onPreferences, onRequestReset, onCancelReset, onReset }: { preferences: ClientPreferences; open: boolean; confirmReset: boolean; onOpenChange: (open: boolean) => void; onPreferences: (preferences: ClientPreferences) => void; onRequestReset: () => void; onCancelReset: () => void; onReset: () => void }): React.ReactElement {
  ...
  const recaptureMicrophoneTest = useCallback(async (): Promise<void> => {
    ...
    try {
      ...
      stopMicrophoneTest();
      setMicrophoneTestStatus("error");
      setMicrophoneTestError(error instanceof Error ? error.message : "Не удалось переключить микрофон");
    }
  }, [preferences.autoGainControl, preferences.echoCancellation, preferences.noiseSuppression, preferences.voiceInputDeviceId, rebuildMicrophoneTestProcessor, stopMicrophoneTest]);
...
function DeviceSelect({ label, devices, value, onChange }: { label: string; devices: MediaDeviceInfo[]; value: string | null; onChange: (value: string | null) => void }): React.ReactElement {
  return <label className="grid gap-1.5 text-xs font-medium text-slate-300">{label}<select value={value ?? ""} onChange={(event) => onChange(event.target.value || null)} className="h-10 rounded-xl border border-white/10 bg-[#26282c] px-3 text-sm text-slate-200 outline-none focus:border-violet-400/50"><option value="">Системное устройство</option>{devices.map((device, index) => <option key={device.deviceId} value={device.deviceId}>{device.label || `${label} ${index + 1}`}</option>)}</select></label>;
}
...
function MicLevelBar({ stream, thresholdDb }: { stream: MediaStream | null; thresholdDb: number }): React.ReactElement {
  ...
  return (
    <div className="mb-3">
      <div role="meter" aria-label="Уровень микрофона" aria-valuemin={0} aria-valuemax={100} aria-valuenow={Math.round(levelPercent)} className="relative h-2 w-full overflow-hidden rounded-full bg-white/15">
        ...
        <div data-testid="mic-level-threshold" className="absolute inset-y-0 w-0.5 -translate-x-1/2 bg-white/90" style={{ left: `${thresholdPercent}%` }} title={`Порог активации: ${thresholdDb} дБ`} />
      </div>
      <p className="mt-1.5 text-[10px] text-slate-600">Уровень сигнала виден во время прослушивания микрофона.</p>
    </div>
  );
}
