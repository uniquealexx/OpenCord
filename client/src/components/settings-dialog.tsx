...
export function SettingsDialog({ preferences, open, confirmReset, onOpenChange, onPreferences, onRequestReset, onCancelReset, onReset, onIdentityReset }: { preferences: ClientPreferences; open: boolean; confirmReset: boolean; onOpenChange: (open: boolean) => void; onPreferences: (preferences: ClientPreferences) => void; onRequestReset: () => void; onCancelReset: () => void; onReset: () => void; onIdentityReset?: (identity: { publicKey: string; fingerprint: string; discriminator: string }) => void }): React.ReactElement {
  const { t } = useI18n();
  const [identity, setIdentity] = useState<{ publicKey: string; fingerprint: string; discriminator: string } | null>(null);
  ...
  function resetIdentity(): void {
    void window.openCord?.identity.reset().then((value) => { setIdentity(value); setIdentityStatus("ready"); setIdentityReset(false); setPublicKeyCopied(false); setPublicKeyCopyFailed(false); onIdentityReset?.(value); }).catch(() => { setIdentity(null); setIdentityStatus("unavailable"); });
  }
...
function DeviceSelect({ label, icon, devices, value, onChange }: { label: string; icon: typeof Mic; devices: MediaDeviceInfo[]; value: string | null; onChange: (value: string | null) => void }): React.ReactElement {
  const { t } = useI18n();
