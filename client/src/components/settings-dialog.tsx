...
import { PushToTalkKeyRow } from "@/components/settings/push-to-talk-key";
      ...
      <DialogContent className="max-w-4xl">
        ...
        <div className="flex min-h-0 flex-col gap-4 md:h-[64vh] md:min-h-[430px] md:flex-row">
          ...
          <div role="tabpanel" aria-label={pageTitle(effectivePage)} className="min-h-0 min-w-0 flex-1 overflow-x-hidden md:overflow-y-auto md:pr-2">
            ...
            {effectivePage === "appearance" && (
              <div className="space-y-4">
                ...
                <Card><RowList>
                  ...
                  <SettingRow title={t.settings.linkPreviews} hint={t.settings.linkPreviewsHint}><Switch checked={preferences.showLinkPreviews} onCheckedChange={(value) => onPreferences({ ...preferences, showLinkPreviews: value })} /></SettingRow>
                </RowList></Card>
            ...
            {effectivePage === "voice" && (
              <div className="space-y-4">
                <Card title={t.settings.inputMode} hint={preferences.voiceInputMode === "push-to-talk" ? t.settings.pushToTalkKeyHint : undefined}><div role="group" aria-label={t.settings.inputMode} className="grid grid-cols-2 gap-1 rounded-lg bg-white/[.04] p-1"><button type="button" aria-pressed={preferences.voiceInputMode === "voice"} onClick={() => updatePreferences({ ...preferences, voiceInputMode: "voice" })} className={cn("rounded-lg px-3 py-2 text-center text-xs", preferences.voiceInputMode === "voice" ? "bg-violet-500 font-semibold text-white" : "text-slate-500 hover:text-slate-200")}>{t.settings.voiceActivation}</button><button type="button" aria-pressed={preferences.voiceInputMode === "push-to-talk"} onClick={() => updatePreferences({ ...preferences, voiceInputMode: "push-to-talk" })} className={cn("rounded-lg px-3 py-2 text-center text-xs", preferences.voiceInputMode === "push-to-talk" ? "bg-violet-500 font-semibold text-white" : "text-slate-500 hover:text-slate-200")}>{t.settings.pushToTalk}</button></div></Card>
                {preferences.voiceInputMode === "push-to-talk" && <Card title={t.settings.pushToTalkKey}><PushToTalkKeyRow value={preferences.pushToTalkKey} onChange={(pushToTalkKey) => updatePreferences({ ...preferences, pushToTalkKey })} /></Card>}
            ...
            {effectivePage === "sensitivity" && (
              <div className="space-y-4">
              {preferences.voiceInputMode === "push-to-talk" && <p role="note" className="rounded-2xl border border-white/7 bg-white/[.025] p-4 text-xs leading-5 text-slate-400">{t.settings.sensitivityPttNote}</p>}
              <Card><RowList>
