...
export function Avatar({ name, image, color = "#7c5cff", size = "md", status, className }: { name: string; image?: string | null; color?: string; size?: "sm" | "md" | "lg" | "xl"; status?: PublicMemberStatus; className?: string }): React.ReactElement {
  ...
  return (
    <span className={cn("relative inline-grid shrink-0 place-items-center overflow-visible rounded-[34%] font-bold text-white", sizes[size], className)} style={{ backgroundColor: color }} aria-label={name}>
      ...
      {status && <span className={cn("absolute -bottom-0.5 -right-0.5 size-3 rounded-full border-[3px] border-[#151925]", status === "online" && "bg-emerald-400", status === "idle" && "bg-amber-400", status === "dnd" && "bg-red-400", status === "offline" && "bg-slate-600")} />}
    </span>
  );
}
