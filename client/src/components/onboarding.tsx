...
export function Onboarding({ language, onLanguageChange, onComplete }: { language: Language; onLanguageChange: (language: Language) => void; onComplete: (profile: LocalProfile) => void }): React.ReactElement {
  ...
  return (
    <main className="relative flex min-h-0 flex-1 items-center justify-center overflow-hidden bg-canvas p-8">
      <div role="group" aria-label={t.onboarding.language} className="absolute right-5 top-5 flex items-center gap-1 rounded-xl border border-white/8 bg-white/[.04] p-1">
        {LANGUAGES.map((option) => (
          <button key={option} type="button" aria-pressed={language === option} onClick={() => onLanguageChange(option)} className={cn("rounded-lg px-3 py-1.5 text-xs font-semibold transition", language === option ? "bg-violet-500 text-white" : "text-slate-500 hover:text-slate-200")}>{LANGUAGE_LABELS[option]}</button>
        ))}
      </div>
      <section className="glass relative grid w-full max-w-5xl overflow-hidden rounded-2xl shadow-[0_24px_70px_rgba(0,0,0,.45)] lg:grid-cols-[1.05fr_.95fr]">
        ...
        <form onSubmit={submit} className="flex flex-col justify-center p-8 sm:p-12">
          ...
          <h1 className="text-3xl font-bold tracking-tight text-white">{t.onboarding.title}</h1>
          <p className="mt-3 text-sm leading-6 text-slate-400">{t.onboarding.description}</p>
          <div className="mt-8 space-y-5">
            <label className="grid gap-2 text-sm font-medium text-slate-300">{t.onboarding.name}<Input autoFocus value={name} onChange={(event) => setName(event.target.value)} placeholder={t.onboarding.namePlaceholder} maxLength={32} /></label>
