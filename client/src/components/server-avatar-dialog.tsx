...
export function ServerAvatarDialog({ server, open, onOpenChange, onSave }: { server: MockServer; open: boolean; onOpenChange(open: boolean): void; onSave(avatar: string | null): boolean }): React.ReactElement {
  ...
  async function chooseAvatar(event: React.ChangeEvent<HTMLInputElement>): Promise<void> {
    ...
    try { setAvatar(await compressServerAvatar(file)); }
    catch (reason) { setError(reason instanceof Error ? reason.message : "Не удалось сжать изображение"); }
