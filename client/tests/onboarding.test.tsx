...
describe("Onboarding", () => {
  it("creates a trimmed local profile with a username and a generated tag", async () => {
    ...
    await user.type(screen.getByRole("textbox", { name: "Username (id)" }), "lina.dev");
    await user.type(screen.getByPlaceholderText(/создаю открытые сообщества/i), "Люблю открытый код");
    await user.click(submit);
    expect(onComplete).toHaveBeenCalledWith(expect.objectContaining({ username: "lina.dev", discriminator: expect.stringMatching(/^\d{4}$/u), bio: "Люблю открытый код", id: "local-user" }));
  });

  it("rejects a username that does not match the allowed pattern", async () => {
    const onComplete = vi.fn();
    const user = userEvent.setup();
    render(<Onboarding language="ru" onLanguageChange={vi.fn()} onComplete={onComplete} />);
    await user.type(screen.getByRole("textbox", { name: "Username (id)" }), "Лина");
    expect(screen.getByRole("button", { name: /создать локальный профиль/i })).toBeDisabled();
